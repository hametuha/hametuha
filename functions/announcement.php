...
class Hametuha_Announcement_Helper{
	...
	public function template_redirect(){
		if(is_singular('announcement')){
			wp_enqueue_script('announcement-helper', get_template_directory_uri().'/js/announcement-helper.js', array('jquery', 'gmap'), $this->version, true);
		}
	}
	
	public function admin_init(){
		add_meta_box('hametuha-announcement', '告知イベントの日時および開催場所<small>（※必要な場合のみ）</small>', array($this, 'metabox_period'), 'announcement', 'normal', 'high');
		if(current_user_can('edit_others_posts')){
			add_meta_box('hametuha-participance', '告知イベントの参加・募集形態<small>（※必要な場合のみ）</small>', array($this, 'metabox_participate'), 'announcement', 'normal', 'high');
		}
		if(
			basename($_SERVER['SCRIPT_FILENAME']) == 'post-new.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'announcement'
				||
			basename($_SERVER['SCRIPT_FILENAME']) == 'post.php' && isset($_GET['post'], $_GET['action']) && get_post_type($_GET['post']) == 'announcement'
		){
			wp_enqueue_script('announcement-helper', get_template_directory_uri().'/js/announcement-helper.js', array('gmap'), $this->version, false);
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th>日時: </th>
					<td>
						<label>開始: <input type="text" class="timepicker" name="annnoucement_start" value="<?php echo esc_attr(get_post_meta($post->ID, self::START_OPTION, true));?>" /></label>
						<label>〜終了: <input type="text" class="timepicker" name="annnoucement_end" value="<?php echo esc_attr(get_post_meta($post->ID, self::END_OPTION, true));?>" /></label>
						<p class="description">
							※終了日を空にすると、無期限になります。
						</p>
					</td>
...
new Hametuha_Announcement_Helper();
 ...
 * @return string 
 */
function get_annnoucement_start($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	$start = get_post_meta($post->ID, Hametuha_Announcement_Helper::START_OPTION, true);
	if($start){
		return $start;
	}elseif(is_limited_announcement($post)){
 ...
 * @return string 
 */
function get_annoucement_end($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	return get_post_meta($post->ID, Hametuha_Announcement_Helper::END_OPTION, true);
}
 ...
 * @param type $post 
 */
function the_announcement_date($post = null){
	$start = get_annnoucement_start($post);
	$end = get_annoucement_end($post);
	if((strtotime($end) - strtotime($start)) <= 60 * 60 * 24){
		//1日なので、時間を出力
		echo mysql2date('Y年m月d日（D）H:i', $start)." 〜 ".  mysql2date('H:i', $end);
	}else{
		//複数の日付なので、時間を出力
		echo mysql2date('Y年m月d日（D）', $start)." 〜 ".mysql2date('Y年m月d日（D）', $end);
	}
}
 ...
 * @return boolean 
 */
function is_limited_announcement($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
 ...
 * @return boolean 
 */
function is_expired_announcement($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	if(is_limited_announcement($post)){
		return time() > strtotime(get_annoucement_end($post));
	}else{
		return false;
	}
}
 ...
 * @return boolean 
 */
function has_place($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	return (boolean)  get_post_meta($post->ID, Hametuha_Announcement_Helper::PLACE, true);
}

/**
 * アドレスを出力する
 * @global object $post
 * @param boolean $for_gmap
 * @param boolean $echo
 * @param object $post 
 * @return string
 */
function the_address($for_gmap = false, $echo = true, $post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	...
	if($for_gmap){
		$address = get_post_meta($post->ID, Hametuha_Announcement_Helper::ADDRESS, true);
	}else{
		$address = get_post_meta($post->ID, Hametuha_Announcement_Helper::ADDRESS, true)
			.' '.get_post_meta($post->ID, Hametuha_Announcement_Helper::BUILDING, true);
	}
	if($echo){
 ...
 * @return void 
 */
function the_announcement_notice($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	$notice = get_post_meta($post->ID, Hametuha_Announcement_Helper::NOTICE, true);
	if($notice){
		$notice = array_map(function($var){
			return preg_replace("/^(https?:\/\/[-_.!~*'()a-zA-Z0-9;\/?:\@&=+\$,%#]+)$/", "<a href=\"$1\" target=\"_blank\">$1</a>", esc_html($var));
		}, preg_split("/\r?\n/", $notice));
 ...
 * @return boolean 
 */
function is_public_announcement($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	...
	$sql = <<<EOS
		...
		ON p.post_author = um.user_id AND um.meta_key = '{$wpdb->prefix}user_level'
		WHERE p.ID = %d AND CAST(um.meta_value AS UNSIGNED) > 4
EOS;
 ...
 * @return boolean 
 */
function is_participating_announcement($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	return (boolean)get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_TYPE, true);
}
 ...
 * @param mixed $post 
 */
function the_participating_period($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	$end = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_END, true);
	if($end){
		$start = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_START, true) ?: $post->post_date;
		echo mysql2date('Y年m月d日（D）', $start)." 〜 ".  mysql2date('Y年m月d日（D）', $end);
	}else{
		echo '無期限';
	}
 ...
 * @return int 
 */
function left_second_to_participate($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
 ...
 * @param mixed $post
 */
function the_participating_condition($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	$condition = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_CONDITION, true);
	if($condition){
		echo implode('<br />', array_map(function($var){
			return preg_replace("/^(https?:\/\/[-_.!~*'()a-zA-Z0-9;\/?:\@&=+\$,%#]+)$/", "<a href=\"$1\" target=\"_blank\">$1</a>", esc_html($var));
		}, preg_split("/\r?\n/", $condition)));
	}else{
		echo '特になし';
 ...
 * @param mixed $post 
 */
function the_participating_cost($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	$cost = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_COST, true);
	if($cost > 0){
		echo number_format_i18n($cost).'円';
	}else{
		echo '無料';
	}
}
 ...
 * @param mixed $post 
 */
function the_participating_limit($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
	}
	$limit = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_LIMIT, true);
	if($limit > 0){
		echo number_format_i18n($limit).'名';
	}else{
		echo '無制限';
	}
}
 ...
 * @return array 
 */
function get_commited_posts($user_id = null, $post = null){
	if(is_null($user_id)){
	...
	if(get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_TYPE, true) != 2){
		return array();
	}
	$post_type = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_POST_TYPE, true);
	$categories = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_CATEGORY, true);
	global $wpdb;
	$wheres = array($wpdb->prepare("(p.post_type = %s)", $post_type));
	//投稿タイプの判別
	if($user_id > 0){
		$wheres[] = "(p.post_status IN ('draft','publish','future'))";
	}else{
	...
	$end = get_post_meta($post->ID, Hametuha_Announcement_Helper::COMMIT_END, true);
	if($end){
		$wheres[] = $wpdb->prepare("(p.post_date BETWEEN %s AND %s)", $start, $end);
	}
	//カテゴリー指定がある場合はJOINとWHEREを改造
	$join = "";
	if(!empty($categories)){
		$join = <<<EOS
...
EOS;
		$wheres[] = "( tt.term_id IN (".implode(",", array_map('intval', $categories)).") )";
	}
	//ユーザーIDが指定されている場合はWHEREを追加
	if($user_id > 0){
		$wheres[] = $wpdb->prepare("(p.post_author = %d)", $user_id);
	}
	$sql = "SELECT p.* FROM {$wpdb->posts} AS p {$join} WHERE ".implode(" AND ", $wheres);
 ...
 * @return array 
 */
function get_required_taxonomies_to_commit($post = null){
	if(is_null($post)){
		global $post;
	}else{
		$post = get_post($post);
 ...
 * 最新の告知を取得する
 * @global wpdb $wpdb
 * @param boolean $public_only trueなら公式告知のみ、falseなら同人の告知、nullなら全部
 * @param int $posts_per_page
 * @return array 
 */
function get_recent_announcement($public_only = null, $posts_per_page = 5){
	global $wpdb;
	$sql = <<<EOS
...
EOS;
	...
	if(!is_null($public_only)){
		if($public_only){
			$wheres[] = 'CAST(um.meta_value AS UNSIGNED) > 4';
		}else{
			$wheres[] = 'CAST(um.meta_value AS UNSIGNED) < 4';
		}
	}
	$sql .= " WHERE ".implode(" AND ", $wheres).' ORDER BY p.post_date DESC LIMIT '.intval($posts_per_page);
	return $wpdb->get_results($sql);
}