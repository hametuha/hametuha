...
<div id="discussion">
	<h2 id="comments">
		...
		<small>
			<span itemprop="<?= is_singular('thread') ? 'answerCount' : 'commentCount' ?>"><?= number_format_i18n(get_comments_number()); ?></span>件
		</small>
	</h2>
	...
	<?php if ( is_user_logged_in() || get_post_type() == 'thread' ): ?>
		...
		<form action="<?php echo $action; ?>" method="post" id="commentform" class="clearfix">
			<p class="comment-text">
				<textarea placeholder="ここにコメントを記載してください" name="comment" id="comment" class="form-control" rows="10" tabindex="4"><? if(isset($_POST['comment'])) echo esc_textarea($_POST['comment']) ?></textarea>
			</p>
			...
			<p class="text-center">
				<input class="btn btn-primary btn-block" name="submit" type="submit" id="submit" tabindex="5" value="<?= esc_attr($label) ?>" onclick="this.value = '送信中...';" />
