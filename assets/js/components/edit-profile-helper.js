...
$( document ).ready( function () {
	...
	let haMediaFrame; // メディアフレーム
	const $picker = $( '.image-picker', '#wpbody-content' ); // プロフィール
	const $img = $picker.find( '.new-img' ); // プレースホルダー
	...
	$picker.on( 'click', '.button-primary', function ( e ) {
		...
		if ( !haMediaFrame ) {
			...
			haMediaFrame.on( 'select', function () {
				// アタッチメントの情報を取得
				const attachment = haMediaFrame.state().get( 'selection' ).first().toJSON();
				// 画像のSRCを取得&設定。小さいサイズがあれば取得し、なければフルサイズ
				// pinkyは add_image_size で追加したオリジナルサイズ
				const src = attachment.sizes.pinky ? attachment.sizes.pinky.url : attachment.sizes.full.url;
				$img.attr( 'src', src );
