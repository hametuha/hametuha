 ...
 * フォームの処理を行う
 *
 * @handle hametuha-form
 * @deps jquery, hametuha-common
 */
...
jQuery( document ).ready( function ( $ ) {
	...
	$( '.validator' ).submit( function ( e ) {
		...
		$( this ).find( '.required' ).each( function ( index, elt ) {
			if ( !$( elt ).val() ) {
				$( elt ).addClass( 'error' );
				const label = $( 'label[for=' + $( elt ).attr( 'id' ) + ']', this );
				if ( label.length ) {
	...
	$( document ).on( 'submit', '#searchBox form', function () {
		const $checked = $( this ).find( 'input[name=post_type]:checked' );
		switch ( $checked.val() ) {
			case 'any':
				// Remove radio
