...
class Announcement extends Singleton {
	...
	public function metabox_participate( $post ) {
		...
		<table class="form-table" id="commit-conditions"
		<?php
		if ( ! get_post_meta( $post->ID, self::COMMIT_TYPE, true ) ) {
		...
		<table class="form-table" id="commit_1"
		<?php
		if ( 1 != get_post_meta( $post->ID, self::COMMIT_TYPE, true ) ) {
			echo ' style="display:none;"';}
		?>
		>
			...
			<tr>
				<th>添付ファイル</th>
				<td><label><input type="checkbox" name="commit_file" id="commit_file" value="1"
				<?php
				if ( get_post_meta( $post->ID, self::COMMIT_FILE, true ) ) {
					echo ' checked="checked"';}
				?>
				 />添付ファイルを必須にする</label></td>
			</tr>
			</tbody>
		</table>
		</table>
		<table class="form-table" id="commit_2"
		<?php
		if ( 2 != get_post_meta( $post->ID, self::COMMIT_TYPE, true ) ) {
			echo ' style="display:none;"';}
		?>
		>
			...
			<tr>
				...
				<td>
					<?php
					$categories = get_post_meta( $post->ID, self::COMMIT_CATEGORY, true ); foreach ( array(
						'post' => 'category',
						'anpi' => 'anpi_cat',
					) as $post_type  => $cateogry ) :
						?>
						<p id="commit_cat_<?php echo $post_type; ?>"
													 <?php
														if ( $post_type != get_post_meta( $post->ID, self::COMMIT_POST_TYPE, true ) ) {
															echo ' style="display:none;"';}
														?>
						>
		...
		<table class="form-table" id="commit_3"
		<?php
		if ( 3 != get_post_meta( $post->ID, self::COMMIT_TYPE, true ) ) {
			echo ' style="display:none;"';}
		?>
...
EOS;
	...
	private function is_datetime( $str ) {
		return (bool) preg_match( '/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/', $str );
	}
}
