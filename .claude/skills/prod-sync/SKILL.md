...
### 0.5. 最新の master から作業ブランチを切り、進行中の作業と衝突しないか確認する

**比較の基準は常に「最新の master」。** 古い master 起点のブランチで実行すると、既にマージ済みの作業をもう一度やり直すことになる。

```bash
git fetch origin
git switch master && git pull
git switch -c chore/prod-sync-$(date +%Y%m%d)
```

そのうえで、**同じ領域を触る PR が開いていないか**を必ず見る。

```bash
gh pr list --state open --limit 10
# composer.json を触っている PR があれば中身を確認する
gh pr diff <番号> | grep -E '^[-+].*wpackagist-plugin|johnpbloch'
```

特に **Dependabot の composer PR には注意**。同じ更新を先回りしていることがあるが、**Dependabot はバージョン制約のスタイルを保存する**ため、固定ピン（`"5.7.7"`）はピンのまま上げる（`"6.1.7"`）。これでは「ピンのせいで更新が止まる」という根本問題が再発する。また **CLAUDE.md で意図的に固定しているバージョンも問答無用で上げる**。

実例（2026-09-14）: Dependabot の #398 が #400 とほぼ同内容（CF7・gianism・query-monitor・wp-pagenavi・コア 7.1）を先行していたうえ、**意図的に 2.1.0 に留めていた hameslack を、壊れているリリース 2.2.0 に上げようとしていた**。

衝突していたら、**どちらを正とするかを先に決める**こと。両者が composer.json / composer.lock の同じ行を触る以上、後からマージするほうは必ず手直しが要る。
