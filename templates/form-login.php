...
<div id="login-body">
	...
	<div class="alert alert-info">
		<p>
			<?= esc_html( $login_name ) ?>さん、あなたのログイン情報を変更しようとしています。これは重要な情報なので、注意書きを理解したうえで行ってください。
		</p>
	</div>
	...
	<form id="change-login-form" method="post" action="<?= $action ?>">
		...
		<div class="form-group">
			...
			<input class="form-control" type="text" readonly value="<?= esc_attr( $login ) ?>"/>
		</div>

		<div class="form-group has-feedback">
			<label for="login_name">新しいログイン名</label>
			<input class="form-control" type="text" id="login_name" name="login_name" data-check="<?= $check_url ?>"
