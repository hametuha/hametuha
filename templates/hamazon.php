...
<div class="form-wrap">
	<form method="GET" action="<?php the_permalink(); ?>">
		<label>
			Amazonを検索:&nbsp;
			<input type="text" value="<?php if(isset($_GET['s'])) echo esc_attr($_GET['s']); ?>" name="s" />&nbsp;
		</label>
		<select name="SearchIndex">
			<?php foreach($wp_hamazon_parser->searchIndex as $k => $v): ?>
			<option value="<?php echo $k; ?>"<?php if((isset($_GET['SearchIndex']) && $_GET['SearchIndex'] == $k) || (!isset($_GET['s']) && $k == 'Books') ) echo ' selected="selected"'; ?>>
				<?php echo $v; ?>
			</option>
			<?php endforeach; ?>
		</select>
...
if(isset($_GET['s']) && !empty($_GET['s'])):
	...
	if($total > 0): ?>
		...
		<ol>
			<?php $counter = 0; foreach($results->Items->Item as $item): $counter++; ?>
			<li>
	...
	<?php else: ?>
		<dl>
		<?php $counter = 0; foreach($hamazon_posts as $p): $counter++;?>
			<dt class="clearfix">
