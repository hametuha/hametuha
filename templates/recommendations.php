...
if ( $args['fill'] && count( $series ) < $args['posts_per_page'] ) {
	foreach ( get_posts( hametuha_series_args( [
...
<ol class="book-recommend">
	...
	<?php foreach ( $series as $post ) : setup_postdata( $post ); ?>
		<li class="book-recommend-item">
			<a class="book-recommend-link" href="<?php the_permalink() ?>">
                ...
                <span class="book-recommend-author"><i class="icon-user"></i> <?= esc_html( hametuha_author_name() ) ?></span>
			</a>
		</li>
