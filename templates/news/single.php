				...
				<article <?php post_class( 'col-xs-12 col-sm-9 main-container' ) ?>>
					...
					<div <?php post_class( 'post-meta' ) ?>>
						...
						<div class="news-author">
							<a class="news-author__link clearfix" href="<?= home_url( sprintf( '/doujin/detail/%s/', get_the_author_meta( 'user_nicename' ) ) ) ?>">
								...
								<small class="news-author__position"><?= hametuha_user_role( get_the_author_meta( 'ID' ) ) ?></small>
					...
					<?php if ( ( $terms = get_the_terms( get_post(), 'nouns' ) ) && ! is_wp_error( $terms ) ) : ?>
						<hr />
						<h2 class="news-keywords__title">キーワード<small>このニュースに出てくる固有名詞</small></h2>
