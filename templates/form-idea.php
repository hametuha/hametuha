<form id="new-idea-form">
	<?php if ( $id ) : ?>
		<input type="hidden" name="post_id" id="new-idea-id" value="<?= esc_attr( $id ) ?>"/>
	<?php endif; ?>
	...
	<div class="form-group">
		...
		<textarea rows="5" class="form-control"
		          ...
		          name="new-idea-content" id="new-idea-content"><?= esc_textarea( $content ) ?></textarea>
	</div>
	<div class="form-group">
		...
		<select class="form-control" name="new-idea-genre" id="new-idea-genre">
			...
			foreach (
				...
				?>
				<option
					value="<?= esc_attr( $term->term_id ) ?>" <?php selected( $term->term_id == $genre ) ?>><?= esc_html( $term->name ) ?></option>
			<?php endforeach; ?>
		</select>
	</div>
	<div class="form-group">
	...
	<div class="row">
		...
		<div class="col-xs-6">
			<input type="submit" class="btn btn-primary btn-block" value="<?= $id ? '更新' : '作成' ?>"/>
		</div>
	</div>
</form>
