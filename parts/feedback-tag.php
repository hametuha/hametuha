...
<?php endif; ?>
<div class="all-tags tag-container
<?php
if ( empty( $tags ) ) {
	echo ' no-tag';}
?>
" id="user-tag-list" data-post-id="<?php the_ID(); ?>">
	<?php foreach ( $tags as $tag ) : ?>
		...
		if ( $tag->owning ) {
			echo 'me';}
		?>
		" href="<?php echo get_tag_link( $tag ); ?>" data-taxonomy-id="<?php echo $tag->term_taxonomy_id; ?>" data-term="<?php echo esc_attr( $tag->name ); ?>" data-number="<?php echo $tag->number; ?>">
			<?php echo esc_html( $tag->name ); ?> (<?php echo $tag->number > 100 ? '100+' : $tag->number; ?>)
			<?php if ( is_user_logged_in() ) : ?>
			<i></i>
			<?php endif; ?>
		</a>
...
<div class="your-tags">
	...
	<?php if ( is_user_logged_in() ) : ?>
		<form id="user-tag-editor" method="post" action="<?php echo home_url( '/user-tags/add/' . get_the_ID() ); ?>/">
