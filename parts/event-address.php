<?php if ( $post->_event_title ) : ?>
	<div class="row news-event">
		...
		<div class="col-sm-12 col-md-6 news-event__info">
			...
			<h2 class="news-event__title"><?php echo esc_html( $post->_event_title ); ?></h2>

			<?php if ( $post->_event_start ) : ?>
				<p class="news-event__date">
					<strong><i class="icon-calendar"></i>
						日時</strong> <?php echo hamenew_event_date( $post->_event_start, $post->_event_end ); ?>
			...
			<?php elseif ( $post->_event_end ) : ?>
				<p class="news-event__date">
					<strong><i class="icon-calendar"></i>
						〆切</strong> <?php echo mysql2date( 'Y年n月j日（D）', $post->_event_end ); ?>
			...
			<?php if ( $post->_event_desc ) : ?>
				<p class="news-event__desc">
					<?php echo nl2br( esc_html( $post->_event_desc ) ); ?>
				</p>
			<?php endif; ?>
			...
			<div class="col-sm-12 col-md-6 news-event__map">
				<div class="news-event__map--inner">
					<iframe width="100%" height="100%" frameborder="0" scrolling="no"
							...
							src="https://www.google.com/maps/embed/v1/place?key=AIzaSyDDZqyowmW69rWqntGmiYRg1u3tira2Wm8&q=<?php echo rawurlencode( $post->_event_address ); ?>"></iframe>
				</div>
			</div><!-- -->
