 ...
 */
add_action(
	'hamethread_after_thread_form',
	function( $args, $default ) {
		if ( $args['post'] ) {
			return;
		}
		?>
...
add_filter(
	...
	function( WP_Error $error, WP_REST_Request $request ) {
		if ( $request->get_param( 'is_private' ) && $request->get_param( 'post_as_anonymous' ) ) {
			$error->add(
				'hidden_thread',
				'匿名かつ非公開だと誰も見られないスレッドになってしまいます。',
				[
					'response' => 400,
					'status'   => 400,
				]
...
add_filter(
	...
	function( array $subscribers, $post_id, $user_id, WP_REST_Request $request ) {
...
add_action(
	...
	function( $post_id, WP_REST_Request $request ) {
 ...
 */
add_filter(
	'hamethread_new_comment_rest_args',
	function( $args, $http_method ) {
		if ( 'POST' === $http_method ) {
			$args['post_as_anonymous'] = [
				'type'              => 'int',
				'description'       => 'Flag to detect post as anonymous.',
 ...
 */
add_action(
	'hamethread_after_comment_form',
	function( $args, $comment = null ) {
		if ( $comment ) {
			return;
		}
		?>
...
add_filter(
	...
	function( array $params, WP_REST_Request $request ) {
		if ( $request->get_param( 'post_as_anonymous' ) && ( $anonymous = hametuha_get_anonymous_user() ) ) {
			$params = array_merge(
				$params,
				[
...
add_action(
	...
	function( $comment_id, array $comment_param, WP_REST_Request $request ) {
		if ( $request->get_param( 'post_as_anonymous' ) && ( $anonymous = hametuha_get_anonymous_user() ) ) {
